...
func _enter_tree():
	...
	efs.filesystem_changed.connect(Callable(self, "_on_filesystem_changed"))

	refresher.request_refresh_plugin.connect(Callable(self, "_on_request_refresh_plugin"))
	refresher.confirm_refresh_plugin.connect(Callable(self, "_on_confirm_refresh_plugin"))
...
func _reload_plugins_list():
	...
	while file:
		var addon_dir = ADDONS_PATH + "/" + file
		if dir.dir_exists(addon_dir) and file != refresher_dir:
			var display_name = file
			var plugin_config_path = addon_dir + "/" + "plugin.cfg"
...
func get_config_path():
	var dir =  EditorPaths.new().get_project_settings_dir()
	var home = dir + "/" + PLUGIN_CONFIG_DIR
	var path = home + "/" + PLUGIN_CONFIG
